...
def add_to_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        cart_item = CartItem.objects.filter(user=current_user, product=product, variations=variation).first()
        ...
        username = f"AnonymousUsername_{uuid.uuid4().hex[:10]}"
        first_name = f"AnonymousFirstName_{uuid.uuid4().hex[:10]}"
        last_name = f"AnonymousLastName_{uuid.uuid4().hex[:10]}"
        anonymous_email = f"AnonymousMail_{uuid.uuid4().hex[:10]}"
        
         # Stockez les détails de l'utilisateur anonyme dans la session
        request.session['anonymous_username'] = username
        request.session['anonymous_first_name'] = first_name
        request.session['anonymous_last_name'] = last_name
        request.session['anonymous_email'] = anonymous_email
        userAno = Account.objects.create(username=username, email=anonymous_email, first_name=first_name, last_name=last_name)
        
        
        cart_item = CartItem.objects.filter(product=product, user=userAno, cart=cart, variations=variation).first()

        if cart_item:
            cart_item.quantity += 1
            cart_item.save()
        else:
            cart_item = CartItem.objects.create(product=product, user=userAno,cart=cart, quantity=1)
            cart_item.variations.add(variation)
