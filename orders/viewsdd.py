...
def place_order(request, total=0, quantity=0):
    ...
    if current_user is not None and current_user.is_anonymous:
    # Get cart items using session cart_id
    ...
    if request.method == 'POST':
        ...
        if orderform.is_valid():
            ...
            # Vérifiez si l'utilisateur anonyme existe dans la session
                ...
                anonymous_first_name = request.session.get('anonymous_first_name')
                anonymous_last_name = request.session.get('anonymous_last_name')
                anonymous_email = request.session.get('anonymous_email')

                # Utilisez les détails de l'utilisateur anonyme s'ils existent
                if anonymous_username and anonymous_email:
                    current_user = Account.objects.create(username=anonymous_username, email=anonymous_email, first_name=anonymous_first_name, last_name=anonymous_last_name)
